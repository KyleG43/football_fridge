...
for team in teams:
  if team not in valid_teams:
    ...
    exit(2)
  scores[team] = {'value': None, 'in_progress': True}
...
def exit_if_no_teams_are_playing():
  if all(not score['in_progress'] for score in scores.values()):
    GPIO.cleanup()
...
while True:
  ...
  for event in response.json()['events']:
    for competitor in event['competitions'][0]['competitors']:
      ...
      in_progress = event['status']['type']['state'] == 'in'
      
      new_scores[team] = {'value': score, 'in_progress': in_progress}
  
  for team, old_score in scores.items():
    if not old_score['in_progress']:
      continue
    ...
    if team in new_scores:
      ...
      if not scores[team]['in_progress']:
        print(f'{team} are not playing')
        exit_if_no_teams_are_playing()
    else:
      scores[team]['in_progress'] = False
